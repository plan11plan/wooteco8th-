...
import static christmas.model.MenuItem.MenuCategory.MAIN;
...
/**
 * 주말 할인 기간 2023.12.1 ~ 2023.12.31 (금요일, 토요일) 보상 메인 메뉴 1개당 2,023원 할인
 */
public class WeekEndDiscount implements DiscountPolicy {
    ...
    public WeekEndDiscount() {
        this.days = List.of(FRIDAY, SATURDAY);
    }
    ...
    public DiscountResult apply(DiscountContext context) {
        ...
        int count = menus.stream()
                .filter(menu -> Objects.equals(menu.menuItem().getCategory(), MAIN.name()))
                .mapToInt(i -> i.count())
                .sum();
